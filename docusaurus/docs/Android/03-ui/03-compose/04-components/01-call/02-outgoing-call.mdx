...
The `OutgoingCall` lets you easily build UI when you're calling or ringing other people in an app. It's used to show more information about the participants you're calling, as well as give you the option to cancel the call before anyone accepts. We provide two versions of the `OutgoingCall`:
...
* **Stateless**: This is a stateless version of the `OutgoingCall`, which doesn't depend on a `ViewModel` and instead depends on pure state from external sources to render its UI.
...
Based on the state, the `OutgoingCall` provides a list of participants, with their avatars and names, or a background with the avatar of the person you're calling, if it's a 1:1 conversation.

Let's see how to show the `OutgoingCall` UI.

## Usage

To use the **bound** `OutgoingCall`, add it to your UI within `setContent()`:
...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContent {
        VideoTheme {
            OutgoingCall(viewModel = callViewModel, onBackPressed = { cancelCall() }) // here
        }
    }
}
...
The `OutgoingCall` is very simple and doesn't offer any customization other than the root `Modifier`. We strongly suggest building custom Outgoing UI if your app requires something that's different from our default.