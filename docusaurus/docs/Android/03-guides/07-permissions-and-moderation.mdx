...
If you use `CallViewModel` with UI components, you can simply set it to the `CallViewModel` like the example below:

```kotlin
// create a CallViewModel.
val factory = CallViewModelFactory(call = streamVideo.call(type = type, id = id))
val vm by viewModels<CallViewModel> { factory }

// set your permissionManager.
vm.setPermissionManager(permissionManager)
```
