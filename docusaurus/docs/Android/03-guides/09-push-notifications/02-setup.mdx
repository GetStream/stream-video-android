...
* `io.getstream.video.android.action.LIVESTREAM`: Action used to go into a livestream. The `activity` that handles this action should show the livestream screen. You can use our [CallContantent](../../04-ui-components/04-call/02-call-content.mdx) component to build your screen.
...
<manifest>
    <application>
        ...
        <activity
            android:name=".LivestreamCallActivity"
            android:exported="false"
            >
            <intent-filter android:priority="1">
                <action android:name="io.getstream.video.android.action.LIVESTREAM" />
            </intent-filter>
        </activity>
    </application>
</manifest>
...
class MyNotificationHandler(private val context: Context) : NotificationHandler {
    ...
    // Called when a Livestream Call arrives.
