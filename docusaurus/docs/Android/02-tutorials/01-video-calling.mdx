...
There are 2 versions of Stream's SDK. "io.getstream:stream-video-android-compose" that includes the video SDK + compose UI components.
And "io.getstream:stream-video-android-core" that only includes the video SDK.
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val client = StreamVideoBuilder(
             ...
             user = user,
             token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoidHV0b3JpYWxAZ2V0c3RyZWFtLmlvIiwiaXNzIjoicHJvbnRvIiwic3ViIjoidXNlci90dXRvcmlhbEBnZXRzdHJlYW0uaW8iLCJpYXQiOjE2ODY2NDk5MDIsImV4cCI6MTY4NzI1NDcwN30.pHQhAXWzhTT7N2xk9JKS01a936QuD9tL7IEtZj1VICI",
        ).build()
        ...
        setContent {
            // step4 - request Android runtime permissions.
            LaunchCallPermissions(call = call)

            // step5 - apply VideTheme
            VideoTheme {
                // step6 - define required properties.
                val participants by call.state.participants.collectAsState()
                val connection by call.state.connection.collectAsState()

                // step7 - render texts that display connection status.
                Box(
...
Next, you should request [Android runtime permissions](https://source.android.com/docs/core/permissions/runtime_perms) that should be required to access physical devices (camera, microphone, and Bluetooth).
You can easily request permissions like the code below:
...
Using the `CallViewModel` and `AbstractCallActivity` enables picture in picture and full screen support.
...
#### C. Allow the picture in picture mode

Next, you should allow the picture in picture mode on your `AndroidManifest.XML` file:
...
And finally, override `pipChanged` method inside your activity:
...
```kotlin
private val onCallAction: (CallAction) -> Unit = { callAction ->
    when (callAction) {
