    ...
    Scaffold(
        ...
        bottomBar = {
            Button(
                ...
                onClick = {
                    lifecycleScope.launch {
                        if (backstage) call.goLive() else call.end()
                    }
                }
            ) {
                Text(
                    text = if (backstage) "Go Live" else "End Livestream",
                    color = Color.White
                )
            }
        }
...
TODO: verify we load this user info
* participant.user.name
* participant.user.image?
* participant.user.custom
* participant.video (in case more users can share video)
