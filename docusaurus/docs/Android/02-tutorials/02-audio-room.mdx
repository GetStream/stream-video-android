...
To keep the tutorial short and simple to follow we've added the client, state and UI straight into the MainActivity.
For a real app you'd typically want to use an Application class for the client and a viewmodel for the state.
...
setContent {
    ...
    VideoTheme {
        ...
        val audioLevel = dominantSpeaker?.audioLevel?.collectAsState()
...
You'll see that the AudioRoom composable hasn't been implemented yet. In `MainActivity` add the following Audio Room composable:
...
The audio room is pretty basic. It needs a "Controls", "Participants" and "Description" composable to work.
...
public fun Participants(
    ...
    Text("coming soon", fontSize = 30.sp)
}
...
public fun Controls(
    ...
    Text("controls", fontSize = 30.sp)
}
```

That's it for the basics. Now when you run your app you'll see the following UI:

The approach is the same for all components. We take the state found in `call.state` and `call.state.participants` and use it to power our UI.
...
public fun Participants(
    ...
    LazyVerticalGrid(
        ...
        items(sortedParticipants.size) { index ->
            val participant = sortedParticipants[index]
...
Now we'll add a pretty ParticipantsAvatar composable
