...
Open up `MainActivity.kt` and replace the MainActivity class with the followign code:
        ...
        setContent {
            VideoTheme {
                ...
                Column() {
                    ...
                    Text("Room: $title - ${participants.value.size} - id ${call.id}", fontSize = 30.sp)
...
setContent {
    VideoTheme {
        ...
        val audioLevel by dominantSpeaker?.audioLevel?.collectAsState() ?: 0f
