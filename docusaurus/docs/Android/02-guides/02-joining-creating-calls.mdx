...
```kotlin
val filters = mapOf("id" to "123")
val sort = listOf(SortParamRequest("username", 1))
val response = call.queryMembers(filters, sort, limit=100)
```
...
| `created_at` | When the user was created | Yes |
| `custom` | You can query custom data using the "custom.myfield" syntax | No |
