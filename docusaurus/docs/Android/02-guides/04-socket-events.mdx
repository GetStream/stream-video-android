...
```kotlin
callClient.addSocketListener(MySignalSocketListener())
```

The socket accepts a different interface type - `SignalSocketListener`, but its signature is almost the same:
...
* **MuteStateChangeEvent**: Notifies the client whether a given user has muted or unmuted their audio or video.
...
```kotlin
callClient.removeSocketListener(mySignalSocketListenerInstance)
```
