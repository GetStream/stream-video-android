...
// default
CallContainer(
    picture: {DefaultPictureInPictureContent(it)}
)
```
...
CallContainer(
    picture: { call ->
        val otherParticipant = call.state.sortedParticipants.collectAsState()

        VideoRenderer(
            modifier = Modifier.aspectRatio(ScreenShareAspectRatio, false),
            video = otherParticipant.value.video,
        )
    }
)
```
...
First start by enabling support for PIP in your AndroidManifest
...
The viewmodel knows if you're in PIP mode or not and automatically renders the right UI:
