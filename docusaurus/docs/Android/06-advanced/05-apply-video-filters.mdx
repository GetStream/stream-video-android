...
We ship two AI filters in a separate library: background blur and virtual background.

<!-- TODO: Add gradle dependency and describe library contents -->

#### Background Blur

Use the `BlurredBackgroundVideoFilter` class to add a blur effect to the video feed. Pass an instance of it to the `videoFilter` call property.

<!-- TODO: Parameters (blur, threshold) -->
...
Pass an instance of `VirtualBackgroundFilter` to the `videoFilter` call property to set an image as the participant's background.

<!-- TODO: Parameters (image) -->

```kotlin
call.videoFilter = VirtualBackgroundVideoFilter(context)
```
...
abstract class BitmapVideoFilter : VideoFilter() {
    fun filter(videoFrameBitmap: Bitmap)
}
```
