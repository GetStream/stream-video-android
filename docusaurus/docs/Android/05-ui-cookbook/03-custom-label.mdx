...
You can simply change the styles of the participant label by giving `RegularVideoRendererStyle` or `ScreenSharingVideoRendererStyle` to UI components, such as `CallContainer`, `CallContent`, `ParticipantsGrid`, and `ParticipantVideo`.

```kotlin
CallContainer( // CallContent or ParticipantsGrid
