...
Stream SDK provides the basic incoming and outgoing Call with the [RingingCallContent](../../04-ui-components/02-ringing-call.mdx) component. We can breaks it down one by one:
...
- `callHeaderContent`: Content shown for the call header, which is built with `CallAppBar`.
- `callDetailsContent`: Content shown for call details, such as call participant information.
- `callControlsContent`: Content shown for controlling call, such as accepting a call or declining a call.
