...
Internally, this component renders a screen sharing session video as a primary screen and observes participants, which are rendered as list of videos.
...
The `ParticipantsScreenSharing` Composable supports zoom for the sharing session like the image below:
