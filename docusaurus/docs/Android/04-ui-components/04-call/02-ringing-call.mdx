...
* **Bound**: This version binds itself to the internal states a `Call`, such as camera and current participants.
* **Stateless**: This is a stateless version of the `RingingCallContent`, which doesn't depend on the internal states of `Call` and instead depends on pure state from external sources to render its UI.
...
To use the **bound** `RingingCallContent`, add it to your UI within `setContent()`:
