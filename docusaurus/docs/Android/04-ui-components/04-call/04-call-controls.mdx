...
1. You observe the required state using the `CallViewModel`. You can build the `ViewModel` yourself, or use any of our predefined `Activity` constructs that hold `CallDeviceState` and pass it to the `ControlActions`.
2. Once you have the state ready, you build the UI. We omitted modifiers here, as you can arrange the components in any way possible, such as the `CallAppBar` and `ControlActions` taking up a fixed size space, while the `CallVideoRenderer` fills the rest of the parent. `CallVideoRenderer` will take the responsibility of representing the main part of the call.
...
If you want to use your own `ViewModel` and use the stateless version of `ControlActions`, you can implement it like the example below:
    ...
    actions = listOf(
        {
            ToggleCameraAction(
                ...
                isCameraEnabled = isCameraEnabled, // or callViewModel.callDeviceState.isCameraEnabled
                onCallAction = onCallAction
            )
        },
        {
            ToggleMicrophoneAction(
                modifier = Modifier.size(60.dp),
                isMicrophoneEnabled = isMicrophoneEnabled, // or callViewModel.callDeviceState.isMicrophoneEnabled
                onCallAction = onCallAction
            )
        },
    ),
...
![Compose CallContainer](../../assets/compose_call_controls.png)
...
![Compose CallContainer](../../assets/compose_call_controls_actions.png)
