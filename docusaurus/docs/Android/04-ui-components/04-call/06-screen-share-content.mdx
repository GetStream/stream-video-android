...
The `ScreenSharingCallVideoRenderer` is a component that allows you to display a video of a screen sharing session. It also includes options to change the UI orientation and to enter or exit the full screen mode. This component is designed for use in video conferencing and other similar applications where users are able to share their entire screens or specific windows.
...
Let's see how to use the `ScreenShareVideoRenderer`.

## Usage

To use the `ScreenShareVideoRenderer` component in your app, you should observe the screen sharing session from the call state and check whether the session is exist. If a screen sharing session is valid, you can render the screen sharing session and all other participants with `ScreenShareVideoRenderer`. If not, you should render a regular video call with [RegularCallVideoRenderer](../05-participants/02-call-regular-video-renderer.mdx).
