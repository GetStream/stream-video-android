...
The `ScreenSharingCallVideoRenderer` is a Composable component that allows you to display a video of a screen sharing session. It also includes options to change the UI orientation and to enter or exit the full-screen mode. This component is designed for use in video conferencing and other similar applications where users are able to share their entire screens or specific windows.
...
Let's see how to use the `ScreenSharingCallVideoRenderer`.

## Usage

To use the `ScreenSharingCallVideoRenderer` component in your app, you should observe the screen sharing session from the call state and check whether the session exists. If a screen sharing session is valid, you can render the screen sharing session and all other participants with `ScreenSharingCallVideoRenderer` Composable. If not, you should render a regular video call with [RegularCallVideoRenderer](../05-participants/02-call-regular-video-renderer.mdx).
...
The `ScreenSharingCallVideoRenderer` Composable supports zoomable functions for the sharing session like the image below:

![Screen Sharing](../../assets/compose_screensharing_zoom.png)

If you use `CallVideoRenderer`, it will render a screen sharing session video if there's a valid screen sharing session. If not, it will render a regular video call.
