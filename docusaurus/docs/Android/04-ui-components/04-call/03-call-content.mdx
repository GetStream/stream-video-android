...
The `CallContent` lets you easily build UI when inside a call. It handles the state and actions of enabling and disabling audio/video, speakerphone or video, as well as information about the call participants. Similar to [RingingCallContent](02-ringing-call.mdx), the `CallContent` provides two variants:
