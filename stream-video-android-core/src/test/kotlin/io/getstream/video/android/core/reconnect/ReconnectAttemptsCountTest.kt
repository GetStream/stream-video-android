 ...
 * increments [io.getstream.video.android.core.Call.reconnectAttempts] according
...
class ReconnectAttemptsCountTest : IntegrationTestBase() {
    ...
    fun `REJOIN increments reconnectAttempts`() = runTest {
        ...
        assertTrue(
            call.reconnectAttempts > 0,
            "Expected reconnect attempts > 0, got ${call.reconnectAttempts}",
        )
    }
    ...
    fun `Multiple reconnect calls accumulate attempts`() = runTest {
        ...
        val attemptsAfterFirst = call.reconnectAttempts
        ...
        assertTrue(
            call.reconnectAttempts >= attemptsAfterFirst,
            "Expected accumulated attempts >= $attemptsAfterFirst, got ${call.reconnectAttempts}",
        )
    }
}
