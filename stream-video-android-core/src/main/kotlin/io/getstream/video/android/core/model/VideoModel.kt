...
public infix fun CallUser.merge(that: CallUser?): CallUser = when (that) {
    null -> this
        ...
        name = that.name.takeUnless { it.isNullOrEmpty() } ?: this.name,
        role = that.role.takeUnless { it.isNullOrEmpty() } ?: this.role,
        imageUrl = that.imageUrl.takeUnless { it.isNullOrEmpty() } ?: this.imageUrl,
        state = that.state merge this.state,
        createdAt = that.createdAt ?: this.createdAt,
        updatedAt = that.updatedAt ?: this.updatedAt,
        teams = ((that.teams ?: emptyList()) + (this.teams ?: emptyList())).distinct(), // TODO nullable: simplify if possible
    )
}
