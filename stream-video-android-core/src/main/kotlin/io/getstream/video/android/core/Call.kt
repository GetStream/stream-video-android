...
public class Call(
    ...
    internal val _session: MutableStateFlow<RtcSession?> = MutableStateFlow(null)
    ...
    suspend fun join(
        ...
        while (retryCount < 3) {
            ...
            if (result is Success) {
                Log.d("Noob", "RTC Session Connected")
    ...
    // Local Debuging
    private fun traceCallerOrCallee() {
        when (state.ringingState.value) {
            is RingingState.Outgoing -> {
                session.value?.sfuTracer?.trace(
                    PeerConnectionTraceKey.CALLER.value,
                    PeerConnectionTraceKey.CALLER.value,
                )
            }
            is RingingState.Incoming -> {
                session.value?.sfuTracer?.trace(
                    PeerConnectionTraceKey.CALLEE.value,
                    PeerConnectionTraceKey.CALLEE.value,
                )
            }
            else -> {}
