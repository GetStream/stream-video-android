...
import io.getstream.video.android.core.notifications.NotificationHandler.Companion.ACTION_END_CALL
...
/**
 * Used to process any pending intents that feature the [ACTION_END_CALL] action. By consuming this
 * event, it ends a call without starting the application UI, notifying other participants that
 * this user left the call. After which it dismisses the originating notification.
 */
internal class EndCallBroadcastReceiver : BroadcastReceiver() {

    val logger by taggedLogger("EndCallBroadcastReceiver")
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (context != null && intent?.action == ACTION_END_CALL) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                when (val rejectResult = streamVideo.call(callCid.type, callCid.id).end()) {
