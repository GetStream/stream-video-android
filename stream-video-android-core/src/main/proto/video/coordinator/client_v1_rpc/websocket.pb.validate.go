...
// Validate checks the field values on WebsocketError with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *WebsocketError) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on WebsocketError with the rules defined
// in the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in WebsocketErrorMultiError,
// or nil if none found.
func (m *WebsocketError) ValidateAll() error {
	return m.validate(true)
}

func (m *WebsocketError) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return WebsocketErrorMultiError(errors)
	}

	return nil
}

// WebsocketErrorMultiError is an error wrapping multiple validation errors
// returned by WebsocketError.ValidateAll() if the designated constraints
// aren't met.
type WebsocketErrorMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m WebsocketErrorMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m WebsocketErrorMultiError) AllErrors() []error { return m }

// WebsocketErrorValidationError is the validation error returned by
// WebsocketError.Validate if the designated constraints aren't met.
type WebsocketErrorValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e WebsocketErrorValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e WebsocketErrorValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e WebsocketErrorValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e WebsocketErrorValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e WebsocketErrorValidationError) ErrorName() string { return "WebsocketErrorValidationError" }

// Error satisfies the builtin error interface
func (e WebsocketErrorValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = WebsocketErrorValidationError{}
