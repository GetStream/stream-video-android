...
# number-as-float keeps JSON numbers as Float. The generator defaults to Double
# since Float truncates float64s, but our generated models are public, so
# widening them is a breaking change. Drop the opt at the next major.
# empty-collection-defaults keeps optional List/Map/Set fields defaulting to an
# empty collection. The generator defaults them to null so unset fields are
# omitted from request bodies, but that changes our public models' defaults.
# Drop the opt at the next major.
...
./build/chat-manager openapi generate-client \
  ...
  --opt api-service-package="$API_SERVICE_PACKAGE" \
  --opt api-service-dir="$API_SERVICE_DIR" \
  --opt model-package="$MODEL_PACKAGE" \
  --opt moshi-adapters-dir="$MOSHI_ADAPTER_DIR" \
  --opt moshi-adapters-package="$MOSHI_ADAPTER_PACKAGE" \
  --opt classes-to-skip="$CLASSES_TO_SKIP" \
  --opt android-sdk="$ANDROID_SDK" \
  --opt number-as-float=true \
  --opt empty-collection-defaults=true \
